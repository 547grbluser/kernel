...
//	s3c_rtc_tickno = 29;
	...
	if (res == NULL) {
		printk(&pdev->dev, "failed to get memory region resource\n");
		return -ENOENT;
	}
	...
	if (s3c_rtc_mem == NULL) {
		printk(&pdev->dev, "failed to reserve memory region\n");
		ret = -ENOENT;
		goto err_nores;
	}
	...
	if (s3c_rtc_base == NULL) {
		printk(&pdev->dev, "failed ioremap()\n");
		ret = -EINVAL;
		goto err_nomap;
	}
